...
describe('Keystone', function () {
	...
	describe('models/', function () {
	  ...
	  describe('#Playlist.js', function () {
		...
		it('should accept a basic playlist (unpublished)', function (done) {
			const Playlist = keystone.list('Playlist');
			const User = keystone.list('User');
			const Program = keystone.list('Program');
			const assocUser = User.model.findOne({ email: '1@test.com' })
		...
		it('should update assoc\'d program when published', function (done) {
			...
			Playlist.model.findOneAndUpdate({ title: 'First Test' }, { isPublished: true }, {},
				function (err) {
					...
					Program.model.findOne({ title: 'Prg1' })
					.exec(function (err, prg) {
						assert.notEqual(undefined, prg.playlists);
						done();
					})
			});
		});
	  ...
	  describe('#Program.js', function () {
		...
		it('should perform bounds checks on programs'); // Is it even possible?
		it('should perform special bounds checks on biweekly programs');
//it('should associate created playlists with itself'); Fulfilled above
		it('should convert times to strings', function () {
			const Program = keystone.list('Program');
			const prg = Program.model.findOne({
				title: 'Test'
			}).exec(function (err, prg) {
				assert.equal(prg.startTimeString, '12:01 AM');
			});
		})
